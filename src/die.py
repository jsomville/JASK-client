...
class Die_Scene(Scene2):
    ...
    def create_gui(self):
        ...
        text = "Re-birth"
        id = "#btn_rebirth"
        self.btn_rebirth = pygame_gui.elements.UIButton(relative_rect=rect, text=text, manager= self.manager, object_id = id)
    ...
    def on_event(self, event):
        ...
        if event.type == pygame_gui.UI_BUTTON_PRESSED:
            if hasattr(event, "ui_element"):
                if event.ui_element == self.btn_rebirth:
                    print("Clicked on Rebirth")
