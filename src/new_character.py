...
class New_character_Scene(Scene2):
    def on_init(self):
    ...
    def create_gui(self):
        ...
        text = "Done"
        id = "#btn_done"
        self.btn_done = pygame_gui.elements.UIButton(relative_rect=rect, text=text, manager= self.manager, object_id = id)
    ...
    def on_event(self, event):
        ...
        if event.type == pygame_gui.UI_BUTTON_PRESSED:
            if hasattr(event, "ui_element"):
                if event.ui_element == self.btn_done:
